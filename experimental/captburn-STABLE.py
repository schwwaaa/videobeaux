...
#     """Paint‑on (word‑reveal) via ASS karaoke. We pack words into reasonable lines, but reveal per word with \k.
...
#     """Roll‑up style: sliding window of N lines. We emit events as the window advances word‑by‑word."""
...
#!/usr/bin/env python3
"""
CAPTBURN v0.2 — vintage titler‑style caption burner
...
Additions in v0.2 (keeping all v0.1 behavior intact):
  ...
  • Position presets: --pos-preset {bc,bl,br,tc,tl,tr,mc,ml,mr}
    ...
    → Automatically sets ASS alignment + margin L/R/V using safe‑area percents
  • Title‑safe margins as percentages: --safe-v 0.07, --safe-h 0.05 (defaults)
...
Retained: pop‑on / paint‑on / roll‑up modes, full stylizing, XY pin, motion.
"""
